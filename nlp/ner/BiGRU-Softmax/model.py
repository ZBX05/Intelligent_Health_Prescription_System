...
class BiGRU(torch.nn.Module):
    ...
    def forward(self,input_ids:Tensor,labels:Tensor) -> tuple:
        ...
        if labels is not None:
            ...
            if loss_mask is not None:
                positive_pos=loss_mask.view(-1)==True
                positive_logits=classifier_output.view(-1,self.num_labels)[positive_pos]
                positive_labels=labels.view(-1)[positive_pos]
                loss=ce(positive_logits,positive_labels)
            else:
                loss=ce(classifier_output.view(-1,self.num_labels), labels.view(-1))
...
def train_epoch(train_loader:DataLoader,model:Any,optimizer:Any,scheduler:Any) -> float:
    ...
    for train_data_dict in tqdm(train_loader):
        ...
        # train_label=train_label.view(train_label.shape[0],train_label.shape[1],1).to(device[0])
        input_ids=train_data_dict["input_ids"].to(device[0])
        optimizer.zero_grad()
        loss,_=model(input_ids=input_ids,labels=train_label)

        train_loss+=loss.sum().item()
        # train_loss+=sum([sum(sentence_loss[:idx]) for sentence_loss,idx in zip(l,padding_starts)])
        #反向传播
        # [sentence_loss[:idx] for sentence_loss,idx in zip(l,padding_starts)]
        loss.sum().backward()
...
def evaluate(data_loader:DataLoader,model:Any) -> dict:
    ...
    with torch.no_grad():
        for data_dict in tqdm(data_loader):
            ...
            batch_output=logits.argmax(dim=2).to('cpu')
            # (batch_size, max_len)
            labels=labels.to('cpu').numpy()
            batch_preds.extend([[idx for idx in indices[:starts]] for indices,starts in zip(batch_output,padding_starts.tolist())])
            # (batch_size, max_len - padding_label_len)
