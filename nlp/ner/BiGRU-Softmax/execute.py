...
def run(data:dict,NERConfig:NERConfig) -> None:
    ...
    val_data=data["val"]
    vocab=Vocabulary(NERConfig.vocab_dir)
    vocab.load_vocab()
    # NERConfig.num_labels=len(labels_to_ids)
    #定义优化器、调度器
    model=BiGRU(vocab.vocab_size(),NERConfig)
    #全部微调
    ...
    train(train_data=train_data,val_data=val_data,model=model,optimizer=optimizer,scheduler=scheduler,
          num_epochs=NERConfig.num_epochs,batch_size=NERConfig.batch_size,labels_to_ids=labels_to_ids,NERConfig=NERConfig,vocab=vocab)
...
def test(data:dict,NERConfig:NERConfig) -> None:
    ...
    vocab=Vocabulary(NERConfig.vocab_dir)
    vocab.load_vocab()
    test_dataset=DataSequence(test_data,labels_to_ids,vocab,NERConfig.labels_padding_value)
