...
def run(data:dict,IRConfig:IRConfig) -> None:
    ...
    #定义优化器、调度器
    model=BertTextCNN(IRConfig,nn.ReLU())
    if IRConfig.full_fine_tuning:
        bert_optimizer=list(model.bert.named_parameters())
        convs_optimizer=list(model.convs.named_parameters())
        ...
        optimizer_grouped_parameters=[
            ...
            {'params':[p for n, p in bert_optimizer if any(nd in n for nd in no_decay)],
             'weight_decay':0.0},
            {'params':[p for n, p in convs_optimizer if not any(nd in n for nd in no_decay)],
             'lr':IRConfig.learning_rate*5,'weight_decay':IRConfig.weight_decay},
            {'params': [p for n, p in convs_optimizer if any(nd in n for nd in no_decay)],
