...
class BertTextRCNN(nn.Module):
    def __init__(self,IRConfig:IRConfig,activation:Any) -> None:
        super(BertTextRCNN,self).__init__()
        ...
        self.pooler=nn.MaxPool1d(IRConfig.bert_max_length)
        self.classifier=nn.Linear(IRConfig.hidden_size*2+IRConfig.gru_embedding_size,IRConfig.num_labels)
        # assert IRConfig.hidden_size*2//IRConfig.pooler_kernel_size>=1
    ...
    def forward(self,input_ids:Tensor,token_type_ids:Tensor=None,attention_mask:Tensor=None,position_ids:Tensor=None,
        ...
        pooler_input=self.activation(torch.cat((gru_output,sequence_output),dim=2))
        pooler_input=pooler_input.permute(0,2,1)
        self.pooler.kernel_size=pooler_input.shape[-1]
        pooled_output=self.pooler(pooler_input).squeeze()
...
def train_epoch(train_loader:DataLoader,model:Any,optimizer:Any,scheduler:Any,loss:Any) -> float:
    ...
    for train_data_dict in tqdm(train_loader):
        ...
        l.sum().backward()
        if(isinstance(model,BertTextRCNN)):
...
def train(train_data:tuple,val_data:tuple,model:BertTextRCNN,optimizer:Any,scheduler:Any,loss:Any,num_epochs:int,
...
Validate Accuracy: {val_acc[-1]:.3f}')
        ...
        if(improve_acc>1e-5):
            ...
            elif(isinstance(model,BertTextRCNN)):
