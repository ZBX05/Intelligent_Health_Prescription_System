...
class BertTextRNN(nn.Module):
    def __init__(self,IRConfig:IRConfig) -> None:
        super(BertTextRNN,self).__init__()
    ...
    def forward(self,input_ids:Tensor,token_type_ids:Tensor=None,attention_mask:Tensor=None,position_ids:Tensor=None,
        ...
        classifier_output=self.classifier(gru_output[:,-1,:])
...
def train_epoch(train_loader:DataLoader,model:Any,optimizer:Any,scheduler:Any,loss:Any) -> float:
    ...
    for train_data_dict in tqdm(train_loader):
        ...
        l.sum().backward()
        if(isinstance(model,BertTextRNN)):
...
def train(train_data:tuple,val_data:tuple,model:BertTextRNN,optimizer:Any,scheduler:Any,loss:Any,num_epochs:int,
...
Validate Accuracy: {val_acc[-1]:.3f}')
        ...
        if(improve_acc>1e-5):
            ...
            elif(isinstance(model,BertTextRNN)):
